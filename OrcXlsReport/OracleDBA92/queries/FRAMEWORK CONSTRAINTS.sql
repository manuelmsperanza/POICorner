...
from user_constraints uc, (select ucc.table_name, ucc.constraint_name, LISTAGG(ucc.column_name,', ') WITHIN GROUP (ORDER BY ucc.position) column_list
from user_cons_columns ucc
group by ucc.table_name, ucc.constraint_name) list_cols
...
from user_constraints uc, (select ucc.table_name, ucc.constraint_name, LISTAGG(ucc.column_name,', ') WITHIN GROUP (ORDER BY ucc.position) column_list
from user_cons_columns ucc
group by ucc.table_name, ucc.constraint_name) a, (select ucc.table_name, ucc.constraint_name, LISTAGG(ucc.column_name,', ') WITHIN GROUP (ORDER BY ucc.position) column_list
from user_cons_columns ucc
group by ucc.table_name, ucc.constraint_name) b
